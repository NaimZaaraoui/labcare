...
export async function GET(request: NextRequest) {
  try {
    ...
    if (format === 'csv') {
      const items = await prisma.auditLogArchive.findMany({
        where,
      ...
      const fileName = `audit_logs_archive_${new Date().toISOString().slice(0, 10)}.csv`;
    ...
    console.error('Error fetching audit archive logs:', error);
    return NextResponse.json({ error: 'Erreur lors du chargement des archives d’audit.' }, { status: 500 });
  }
}
